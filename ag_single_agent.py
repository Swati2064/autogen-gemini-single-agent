...
# Page Config
st.set_page_config(
    page_title="Single Agent - AutoGen 0.4",
    page_icon="🌱",
    layout="centered"
)
...
if not gemini_key:
    st.warning(
        "⚠️ Please provide a GEMINI_API_KEY in your `.env` file or sidebar."
    )
    st.stop()
...
async def run_single_agent(prompt: str, api_key: str):
    ...
    mentor = AssistantAgent(
        ...
        system_message=(
            "You are a friendly technical educator explaining concepts "
            "to a non-technical audience. "
            "Avoid code or heavy jargon. "
...
if st.button("🚀 Ask Mentor", type="primary"):
    ...
    with st.spinner("Mentor is generating explanation..."):
        ...
        try:
            result = asyncio.run(
                run_single_agent(user_prompt, gemini_key)
            )
